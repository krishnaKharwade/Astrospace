# =============================
# ASTROSPACE FINAL (ERROR-FREE VERSION)
# =============================
...
oxy = st.sidebar.number_input("Atomic Oxygen", 1e8, 1e10, 1e9)
dur = st.sidebar.number_input("Mission Duration", 1.0, 20.0, 5.0)

st.sidebar.header("⚙️ Constraints")

strength = st.sidebar.number_input("Strength", 50, 3000, 500)
fatigue = st.sidebar.number_input("Fatigue", 1000, 10000000, 100000)
weight = st.sidebar.number_input("Weight Limit", 1, 1000, 100)
cost = st.sidebar.number_input("Cost", 1, 2000, 100)

# -----------------------------
# FIXED FEATURE SCHEMA (MOST IMPORTANT FIX)
# -----------------------------
FEATURE_COLUMNS = [
    "temp","rad","oxy","dur",
...
def generate_data(n=300):
    ...
    for _ in range(n):
        ...
        s = np.random.uniform(50,3000)
        f = np.random.uniform(1e3,1e7)
        w = np.random.uniform(10,1000)
        c = np.random.uniform(5,2000)
...
# 🔥 CRITICAL FIX: ALWAYS USE SAME ORDER
X = df[["temp","rad","oxy","dur","strength","fatigue","weight","cost","material_enc"]]
y = df["score"]
...
# -----------------------------
def clustering():
    features = df_materials.select_dtypes(include=[np.number])
    ...
    df_materials["PC1"] = p[:,0]
    df_materials["PC2"] = p[:,1]
...
if st.sidebar.button("🚀 Analyze"):
    ...
    # BAR
    st.plotly_chart(px.bar(df_res, x="Score", y="Material", orientation="h"))

    # PIE (FIXED)
    st.subheader("🥧 Distribution")
    st.plotly_chart(px.pie(df_res, names="Material", values="Score"))

    # FEATURE IMPORTANCE
    st.subheader("📊 Feature Importance")
    st.plotly_chart(px.bar(feature_importance(), x="Importance", y="Feature", orientation="h"))

    # CLUSTERING
    st.subheader("🧬 Clustering")
    cdf = clustering()
    st.plotly_chart(px.scatter(cdf, x="PC1", y="PC2", color="Cluster", text="Material"))
